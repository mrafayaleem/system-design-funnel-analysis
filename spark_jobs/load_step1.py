...
spark = SparkSession.builder.appName('step1 table extract').getOrCreate()
...
def table_to_parquet(df):
    ...
    df.write.parquet(
        '/Users/aleemr/powerhouse/interviews/jerry-coding-challenge/spark_jobs/datalake/step1-{}'.format(
            file_suffix
        ))


def main():
    table_to_parquet(load_table("step1", step_schema()))
