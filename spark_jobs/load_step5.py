...
spark = SparkSession.builder.appName('step5 table extract').getOrCreate()
...
def table_to_parquet(df):
    ...
    df.write.parquet(
        '/Users/aleemr/powerhouse/interviews/jerry-coding-challenge/spark_jobs/datalake/step5-{}'.format(
            file_suffix
        ))


def main():
    table_to_parquet(load_table("step5", step_schema()))
