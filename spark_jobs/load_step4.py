...
spark = SparkSession.builder.appName('step4 table extract').getOrCreate()
...
def table_to_parquet(df):
    ...
    df.write.parquet(
        '/Users/aleemr/powerhouse/interviews/jerry-coding-challenge/spark_jobs/datalake/step4-{}'.format(
            file_suffix
        ))


def main():
    table_to_parquet(load_table("step4", step_schema()))
